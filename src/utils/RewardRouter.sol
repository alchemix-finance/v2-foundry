...
contract RewardRouter is IRewardRouter, Ownable {
    ...
    function distributeRewards(address vault) external {
        ...
        IRewardCollector(rewardCollectors[vault].rewardCollectorAddress).claimAndDonateRewards(vault, IRewardCollector(rewardCollectors[vault].rewardCollectorAddress).getExpectedExchange(vault) * slippageBPS / BPS);
    }
