...
contract UpgradeEIP7702CompatibilityScript is Script {
    ...
    function run() public {
        ...
        // Impersonate the owner for the upgrade call
    ...
    function revertUpgrade() public {
        ...
        // confirm addresses are not whitelisted
        require(!IWhitelist(ALCHEMIST_ALETH_WHITELIST_ADDRESS).isWhitelisted(NEW_ATOKEN_GATEWAY_ALETH), "New ATokenGateway for alETH is still whitelisted");
        require(!IWhitelist(ALCHEMIST_ALETH_WHITELIST_ADDRESS).isWhitelisted(NEW_WETH_GATEWAY), "New WETHGateway is still whitelisted");
        require(!IWhitelist(ALCHEMIST_ALETH_WHITELIST_ADDRESS).isWhitelisted(NEW_AUTOLEVEREGE_METAPOOL), "New AutoleverageCurveMetapool is still whitelisted");
        require(!IWhitelist(ALCHEMIST_ALETH_WHITELIST_ADDRESS).isWhitelisted(NEW_AUTOLEVEREGE_FACTORYETHPOOL), "New AutoleverageCurveFactoryethpool is still whitelisted");
        require(!IWhitelist(ALCHEMIST_ALUSD_WHITELIST_ADDRESS).isWhitelisted(NEW_ATOKEN_GATEWAY_ALUSD), "New ATokenGateway for alUSD is still whitelisted");
