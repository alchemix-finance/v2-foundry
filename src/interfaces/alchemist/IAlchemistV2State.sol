...
interface IAlchemistV2State {
    ...
    struct UnderlyingTokenParams {
        // The number of decimals the token has. This value is cached once upon registering the
        // token so it is important that the decimals of the token are immutable or the system
        // will begin to have computation errors.
        uint8 decimals;
        // A coefficient used to normalize the token to a value comparable to the debt token.
        // For example, if the underlying token is 8 decimals and the debt token is 18 decimals
        // then the conversion factor will be  10^10. One unit of the underlying token will be
        // comparably equal to one unit of the debt token.
    ...
    struct YieldTokenParams {
        ...
        // The adapter used by the system to wrap, unwrap, and lookup the conversion rate of this
        // token into its underlying token.
        address adapter;
        // The maximum percentage loss that is acceptable before disabling certain actions.
        uint256 maximumLoss;
        // The maximum value of yield tokens that the system can hold, measured in units of the
        // underlying token.
        uint256 maximumExpectedValue;
        // The percent of credit that will be unlocked per block. The representation of this value
        // is a 18 decimal fixed point integer.
        uint256 creditUnlockRate;
        // The current balance of yield tokens which are held by users.
        uint256 activeBalance;
        // The current balance of yield tokens which are earmarked to be harvested by the system at
        // a later time.
        uint256 harvestableBalance;
        // The total number of shares that have been minted for this token.
        uint256 totalShares;
        // The expected value of the tokens measured in underlying tokens. This value controls how
        // much of the token can be harvested. When users deposit yield tokens, it increases the
        // expected value by how much the tokens are exchangeable for in the underlying token.
        // When users withdraw yield tokens, it decreases the expected value by how much the tokens
        // are exchangeable for in the underlying token.
        ...
        // The total accrued weight. This is used to calculate how much credit a user has been
        // granted over time. The representation of this value is a 18 decimal fixed point integer.
    ...
    ///
    /// @dev The order of the entries returned by this function is not guaranteed to be consistent
    ///      between calls.
    ///
    ...
    ///
    /// @return If the underlying token is supported.
    ...
    /// @return rate         The rate at which the the current limit increases back to its maximum
    ///                      in tokens per block.
    ...
    /// @notice Gets current limit, maximum, and rate of the liquidation limiter for
    ///         `underlyingToken`.
    ///
    /// @param underlyingToken The address of the underlying token.
    ///
    /// @return currentLimit The current amount of underlying tokens that can be liquidated.
    /// @return rate         The rate at which the function increases back to its maximum limit
    ///                      (tokens / block).
