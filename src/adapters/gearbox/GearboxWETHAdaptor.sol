...
contract GearboxWETHAdaptor is ITokenAdapter {
	...
	function price() external view override returns (uint256) {
		//get the price of Yield Token in WETH
	...
	function wrap(uint256 amount, address recipient) external override returns (uint256) {
		...
		//approve the transfer of tokens to the Gearbox contract
		TokenUtils.safeApprove(underlyingToken, token, amount);
		//deposit the tokens into the Gearbox contract
