...
import { IEthStableMetaPool, N_COINS as NUM_META_COINS } from "./interfaces/external/curve/IEthStableMetaPool.sol";
...
contract EthAssetManager is Multicall, MutexLock, IERC20TokenReceiver {
  ...
  ///
  /// @return minted The number of meta pool tokens minted.
  function mintMetaPoolTokens(MetaPoolAsset asset, uint256 amount) external lock onlyOperator returns (uint256 minted) {
  ...
  function flush(uint256[NUM_META_COINS] calldata amounts) external lock onlyOperator returns (uint256) {
  ...
  ///
  /// @return The amount of meta pool tokens deposited into convex.
  function flush(MetaPoolAsset asset, uint256 amount) external lock onlyOperator returns (uint256) {
  ...
  function sweepEth(uint256 amount) external lock onlyAdmin returns (bytes memory result) {
  ...
  function _mintMetaPoolTokens(MetaPoolAsset asset, uint256 amount) internal returns (uint256 minted) {
    ...
    uint256 value = asset == MetaPoolAsset.ETH ? amounts[uint256(MetaPoolAsset.ETH)] : 0;
