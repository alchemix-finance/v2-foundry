...
import { FuseTokenAdapterV1, InitializationParams as AdapterInitializationParams } from "../adapters/fuse/FuseTokenAdapterV1.sol";
...
contract FuseTokenAdapterV1Test is DSTestPlus, stdCheats {
	...
	function setUp() external {
		...
		adapter = new FuseTokenAdapterV1(
			AdapterInitializationParams({
	...
	function testRoundTrip(uint256 amount) external {
		hevm.assume(amount >= 10**SafeERC20.expectDecimals(adapter.underlyingToken()) && amount < type(uint96).max);
