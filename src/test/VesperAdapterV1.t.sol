...
import { VesperAdapterV1, InitializationParams as AdapterInitializationParams } from "../adapters/vesper/VesperAdapterV1.sol";
...
contract VesperAdapterV1Test is DSTestPlus, stdCheats {
	...
	function testRoundTrip(uint256 amount) external {
		hevm.assume(amount >= 10**SafeERC20.expectDecimals(address(weth)) && amount < type(uint96).max);
