...
import { ThreePoolAssetManager, ThreePoolAsset, MetaPoolAsset, SLIPPAGE_PRECISION, CURVE_PRECISION, InitializationParams as ManagerInitializationParams } from "../ThreePoolAssetManager.sol";
...
contract ThreePoolAssetManagerTest is DSTestPlus, stdCheats {
  ...
  function setUp() external {
    manager = new ThreePoolAssetManager(
      ManagerInitializationParams({
        admin: address(this),
  ...
  function testCalculateRebalanceAlUSD() external {
    ...
    (uint256 delta, bool add) = manager.calculateRebalance(MetaPoolAsset.ALUSD, ThreePoolAsset.DAI, 1.0e18);
  ...
  function testCalculateRebalance3Pool() external {
    ...
    (uint256 delta, bool add) = manager.calculateRebalance(MetaPoolAsset.THREE_POOL, ThreePoolAsset.DAI, 1.0e18);
  ...
  function testReclaimThreePoolAsset() external {
    ...
    hevm.expectCall(
      ...
      abi.encodeWithSelector(IERC20TokenReceiver.onERC20Received.selector, address(dai), 1e18)
    );
