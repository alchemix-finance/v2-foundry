...
contract TransmuterBuffer is ITransmuterBuffer, AccessControl, Initializable {
	...
	function setWeights(
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			...
			if (weightToken != debtToken) {
				IAlchemistV2.YieldTokenParams memory params = IAlchemistV2(alchemist).getYieldTokenParameters(
					yieldToken
				);
	...
	function refreshStrategies() public override {
		address[] memory supportedYieldTokens = IAlchemistV2(alchemist).getSupportedYieldTokens();
		address[] memory supportedUnderlyingTokens = IAlchemistV2(alchemist).getSupportedUnderlyingTokens();
	...
	function _getTotalBuffered(address yieldToken) internal view returns (uint256) {
		...
		IAlchemistV2.YieldTokenParams memory params = IAlchemistV2(alchemist).getYieldTokenParameters(yieldToken);
		uint256 tokensPerShare = IAlchemistV2(alchemist).getUnderlyingTokensPerShare(yieldToken);
	...
	function _alchemistDeposit(address token, uint256 amount) internal {
		IAlchemistV2(alchemist).depositUnderlying(token, amount, address(this), 0);
	}
